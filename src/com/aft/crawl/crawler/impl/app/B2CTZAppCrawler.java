 ...
 * 酷航航空 app  
 */
public class B2CTZAppCrawler extends Crawler {
	
	
	private final static String queryUrl = "https://scootapi.themobilelife.com/BookingManager.svc";
	
	public B2CTZAppCrawler(String threadMark) {
		super(threadMark, "0");
	}
...
//		System.out.println(httpResult);
		...
		for(int jd=0;jd<journeyDateMarkets.getLength();jd++){
			...
			for(int i=0;i<journeys.getLength();i++){
				...
				for(int sg = 0;sg<segments.getLength();sg++){
					...
					if(paxFare != null && paxFare.hasChildNodes()){
	...
	protected boolean needToChangeIp(String httpResult, org.jsoup.nodes.Document document, Object jsonObject, String returnType) throws Exception {
		if(httpResult!=null && httpResult.contains("403 Forbidden"))return true;
...
//		String result = req.request(null);
		...
		headerMap.put("User-Agent", "Scoot/1.6.11 Android/4.4.2");
		headerMap.put("User-Platform", "android");
		headerMap.put("Host", "scootapi.themobilelife.com");
		...
		GetAvailabilityEntity entity = new GetAvailabilityEntity(sig, this.getJobDetail().getDepCode(),this.getJobDetail().getDesCode(),beginDate,endDate, "JPY");
